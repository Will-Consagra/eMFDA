...
parser.add_argument("--rep", action="store", type=int,  default = 0,
				help="Replication to analyze.")
...
n_marginal_basis_dims = (11,) ## Fourier basis rank
K_t = (10, 20)
alpha = 7e-1
## Model params 
Km = (8, 15, 25) 
...
if MODE == "simulate":
	...
	for n_marginal_basis in n_marginal_basis_dims:
	    for K in K_t:
	        random_tensor_distribution(n_marginal_basis=n_marginal_basis, K=K, nmode=nmode, alpha=alpha)
	        ## Simulate replicated datasets corresponding to the desired states of nature  
	        rt_file = "md_%s__nmode_3__K_%s__alpha_%s.pkl" % (n_marginal_basis, K, alpha)
	        for sigma2 in sigma2s:
	            for N in Nsamps:
	                for nd in Nds:
	                    dirname = "sigma2_%s__nd_%s__N_%s" % (sigma2, nd, N)
	                    create_replications(Nreps, N, nd, sigma2, dirname, rt_file)
	##Prepare data for sandwhich smoother in simulation.R
	for n_marginal_basis in n_marginal_basis_dims:
	    for K in K_t: 
	        rt_dir = "md_%s__nmode_3__K_%s__alpha_%s" % (n_marginal_basis, K, alpha)
	        for sigma2 in sigma2s:
	            for N in Nsamps:
	                for nd in Nds:
...
## pass in replication to analyze 
	...
	for n_marginal_basis in n_marginal_basis_dims:
	    for K_true in K_t:
	        rt_dir = "md_%s__nmode_3__K_%s__alpha_%s"%(n_marginal_basis, K_true, alpha)
	                        ...
	                        for K in Km:
	                            for lambda_ in lambda_grid:
	                                ...
	                                try:
	                                    ...
	                                    print("here")
	                                    print(e)
	                                    print(K_true, sigma2, N, nd, mb, K, lambda_)
	                               
	                    with open(os.path.join(DATA_DIR, "analysis_mpb", rt_dir, samp_dir, "results_%s.pkl"%replindex), "wb") as pklfile:
	                        data = pickle.dump(results, pklfile)
